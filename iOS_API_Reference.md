...
名称 | 类型 | 说明
--------- | ------- | -----------
appkey | NSString* | 在云巴 Portal 申请到的应用的 [AppKey][3]。
option | YBSetupOption* | 选项。可包含用于获取订阅权限的密钥 subscribeKey，用于获取发布权限的密钥 publishKey，用于获取管理权限的密钥 secretKey（切勿外泄）和用于 [Access Manager][1] 模块中权限管理的动态密钥）authorizeKey。
...
qosLevel | NSString* | 服务质量等级。具体参考 [QoS][11] 说明。
...
data | NSData* | 待发布的消息。
option | YBPublishOption* | 选项。可包含 [QoS][11] 和 retained（暂未支持）属性。
resultBlock | YBResultBlock | API 回调接口。可通过返回的 BOOL succ 判断结果成功与否；通过 NSError *error 获取错误信息。
...
### 功能
`publish2`是`publish`的升级版，二者都用于向某个 [频道][5] 发布消息，`publish2`可以带有其他参数，如 APN 选项。发布成功后，所有订阅此频道的客户端都会收到消息。
...
### 功能
`publish2ToAlias`是`publishToAlias`的升级版，二者都用于向 [别名][6] 发布一对一的消息，`publish2ToAlias`可以带有其他参数，如 APN 选项。发布成功后，该别名的客户端会收到消息。
...
### 功能
订阅某 [频道][5] 下用户行为的事件通知，事件包括上线、下线、订阅频道和取消订阅频道。
...
### 功能
取消订阅某 [频道][5] 下用户行为的事件通知，事件包括上线、下线、订阅频道、取消订阅频道。
...
### 功能
获取某 [频道][5] 下的所有订阅者的 [别名][6] 列表，别名个数及状态。
...
### 功能
App 将 Device Token 存储在 YunBa 的云端，即可[通过 YunBa 发送 APNs 通知](http://yunba.io/docs2/yunba_ios_messaging)。
...
[1]: https://github.com/yunba/docs/blob/master/yam_API.md
