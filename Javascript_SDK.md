...
这里，需要在云巴 [Portal][4] 上创建新应用，用获取到的 [AppKey][3] 创建 Yunba 实例。
...
如果你想接收一个频道的消息，你得先使用`subscribe()`方法订阅该 [频道][5]，
...
与`connect()`功能一致，不同的是此接口使用特定的会话 ID 进行连接，连接后的会话状态与上次连接一致（包括 [离线消息][1]、已订阅的 [频道][5] 和 [别名][6]）。
...
名称 | 类型 | 说明
--------- | ------- |  -----------
cb | Function | 参数必选。通过该回调函数监听所订阅 [频道][5] 的推送消息。传递回来的参数 data 是一个 object，包含消息频道（data.topic）与消息内容（data.msg）。如果消息为 [presence][2] 消息，data 中会多一个 presence 字段，其中包含 action、[alias][6] 和 timestamp。
...
增加订阅一个 [频道][5]。成功订阅后，可以收到来自该频道的消息。新增订阅不会影响已有的订阅。
...
名称 | 类型 | 说明
--------- | ------- |  -----------
obj | Object |  参数必选。obj 包含 obj.topic 和 obj.qos 两个字段。obj.topic 表示准备收听的频道；obj.qos 为可选，表示服务质量等级，有三种取值：“0”表示最多送达一次；“1”表示最少送达一次；“2”表示保证送达且仅送达一次。默认为“1”。详见 [QoS][11] 的说明。
...
监听某 [频道][5] 下所有用户的上、下线及订阅、取消订阅该频道的事件通知。调用成功后，可以收到所有用户的状态变化。

注意：`subscribe_presence` 只能监听已经设置了 [别名][6] 的客户端，未设置 别名 的客户端不会被监听。
...
名称 | 类型 | 说明
--------- | ------- | -----------
obj | Object | 参数必选。目前版本只要求 obj 包含一个属性字段为 topic，即准备取消收听的 [频道][5]。
...
取消对某 [频道][5] 下所有用户上、下线及订阅、取消订阅该频道的事件通知的监听。
...
名称 | 类型 | 说明
--------- | ------- | -----------
obj    | Object | 参数必选。obj 含有四个属性字段，分别为 目标 [频道][5]（obj.topic:string）、消息体（obj.msg:string）、MessageId（obj.messageId:number/string）和 [消息级别][11]（obj.qos:number），其中 obj.qos 为可选，表示服务质量等级，有三种取值：“0”表示最多送达一次；“1”表示最少送达一次；“2”表示保证送达且仅送达一次。默认为“1”。详见 [QoS][11] 的说明。
...
名称 | 类型 | 说明
--------- | ------- | -----------
obj    | Object | 参数必选。obj 含有三个属性字段，分别为要发送的目标 [频道][5]（obj.topic:string）、消息体（obj.msg:string）和 [扩展参数](#扩展参数说明)（obj.opts:dict）。
cb    | Function | 参数可选。不管消息发布是成功或失败都会回调此函数。传递回的参数有 success、msg。success 值为 true 表示消息发布成功，否则发送失败。如果发送成功返回含有 messageId 的 msg，发送失败返回错误消息 msg。
...
向某个 [别名][6] 发布消息。发布成功后，该别名的客户端会收到消息。用于实现点对点的消息发送。
...
可以通过`get_state()`查看传入的用户 [别名][6] 的在线状态。
...
查询用户订阅的 [频道][5] 列表。如果传入参数 alias，是获取目标用户（[别名][6]）的频道列表；如果不传入参数 alias，则是获取当前用户的频道列表。
...
名称 | 类型 | 说明
--------- | ------- | -----------
alias | String | 用户设置的 [别名][6] 信息，只支持英文数字下划线，长度不超过 50 个字符。
...
通过调用此函数可以获取该 topic 下所有订阅用户的 [别名][6]。
