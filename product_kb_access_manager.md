...
**应用开发者们需要建立自己的权限管理服务器**，一面向云巴请求权限，一面将不同的权限授予其不同的用户。开启权限管理后，用户的所有 Pub/Sub 等操作都会需要经过云巴权限管理模块的核查，认证成功后才会执行。
...
开启权限管理后，大致的通信流程如下（其中，用户的服务器和客户端之间的通信逻辑由用户自己决定）：
...
**开启权限管理功能后（目前还不支持在 Portal 进行开关），用户客户端的所有的订阅（读）、发布（写）操作都默认被禁止，只有被用户的服务器授权才能进行操作。**
