...
- 结算日生成账单后，如果存在欠费，应用还可以正常使用 20 天，之后会进入免费受限状态（免费受限状态：仅可享受免费档次，超过免费日活限制的那部分客户端无法连接云巴服务器），该状态下再过三个月不支付就会注销应用。

- 日活超过基础版最高档次时，用户需要联系商务开通专业版，如未开通，用户可以正常使用 30 天，此后会进入档次内受限状态（档次内受限状态：可以正常使用现有档次，超出本档次的日活限制的客户端无法连接云巴服务器）。
